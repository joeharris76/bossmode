...
CREATE INDEX idx_tasks_state_priority ON tasks(state, priority DESC, created_at);
CREATE INDEX idx_feedback_recurrence_key ON feedback(recurrence_key, created_at);
CREATE INDEX idx_run_turns_run_ordinal ON run_turns(run_id, ordinal);
...
INSERT INTO tasks(
    ...
    'task_v4', 'Historical task', 'Migrate', 'Preserve every record',
    'failed', '{}', 'then', 'then'
);
INSERT INTO runs(
    ...
    'run_v4', 'task_v4', 'worker', 'finished', 'failed', 'historical run',
    '[]', 0, 'then', 'then'
);
INSERT INTO herdr_bindings(
    run_id, herdr_session, worker_name, agent_kind, status, bound_at, reconciled_at
) VALUES ('run_v4', 'bossmode', 'worker_v4', 'codex', 'stale', 'then', 'then');
INSERT INTO run_turns(
    id, run_id, ordinal, purpose, prompt, prompt_digest, artifact_path, status,
    summary, result_json, started_at, finished_at
) VALUES (
    'turn_v4', 'run_v4', 1, 'task', 'historical prompt', 'digest',
    '.bossmode/turns/turn_v4.json', 'succeeded', 'preserved',
    '{"turn_id":"turn_v4","status":"succeeded","summary":"preserved","artifacts":[]}',
    'then', 'then'
);
INSERT INTO evaluations(
    id, task_id, run_id, evaluator, passed, evidence, created_at
) VALUES ('eval_v4', 'task_v4', 'run_v4', 'reviewer', 0, 'historical evidence', 'then');
INSERT INTO feedback(
    id, task_id, run_id, kind, recurrence_key, content, created_at
) VALUES (
    'feedback_v4', 'task_v4', 'run_v4', 'failure',
...
INSERT INTO promotions(
    ...
    'promotion_v4', 'historical.failure', 'control', 'proposed',
