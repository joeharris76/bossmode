...
- **Special-Effort Relay:** Before dispatching a charter that authorizes the user-gated Tier 1 effort levels in `shared-agent-execution/SKILL.md`, the Executive reconciles a direct current-task user instruction granting it, and records that request verbatim with its exact source in the charter as evidence; the charter itself does not create `user` origin. The paired Manager verifies that record and confirms no later correction supersedes it before acting on it, and does not independently query the user.
