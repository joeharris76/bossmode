...
- **Special-Effort Relay:** Before acting on the user-gated Tier 1 effort levels in `shared-agent-execution/SKILL.md`, the Executive reconciles a direct current-task user instruction granting that specific effort level and records the request verbatim with its exact source — in the charter when granted before dispatch, or in a correction delta when granted later — as evidence; neither the charter nor the correction delta itself creates `user` origin. The paired Manager verifies that record and confirms no later correction supersedes it before acting on it, and does not independently query the user.
...
- **Policy vs User:** Repository policy may constrain the method, order, or completeness of authorized work, but cannot grant or expand authority. A repository-policy requirement for a second same-key read-only reviewer (see manager.md) is such a method constraint on already-authorized review, not a grant or expansion of repository-write authority. Treat a policy-required step outside authorized boundaries as required-but-pending: report it, finish independent authorized steps, and stop only when it is the next dependency.
